...
# from ..charts import BarChartWidget  # se precisar do gráfico de barras
# mensal
...
class MainWindow(QMainWindow):
    def __init__(self, repo: TransacaoRepository):
        ...
        # 20 cores harmonizadas
        self._palette = list(cm.get_cmap("tab20").colors)
        self._color_idx = 0
        ...
        self.lbl_saldo.setStyleSheet(
            "color: #eaeaea; font-weight: 700;font: Segoe UI, sans-serif;")
        ...
        # top margin p/ respirar da tabela
        ...
        self.charts_title.setStyleSheet(
            "color: #eaeaea; padding: 8px; font: Segoe UI, sans-serif;")
        ...
        # Inicializa
        # agora que toda a UI foi criada, aplica o período inicial (isto chama
        # _recarregar internamente)
    ...
    def _recarregar(self):
        ...
        if (self._inicio is not None) and (self._fim is not None):
            transacoes = self.repo.listar_filtrado(
                self._inicio, self._fim, cat)
        else:
            transacoes = self.repo.listar_filtrado(None, None, cat)

        if (self._inicio is not None) and (self._fim is not None):
            dados = self.repo.soma_por_categoria_periodo(
                TipoTransacao.DESPESA, self._inicio, self._fim)
        else:
        ...
        colors = [self._color_for(cat) for cat in labels] if hasattr(
            self, "_color_for") else None
    ...
    def _color_for(self, categoria: str):
        if categoria not in self._cat_colors:
            self._cat_colors[categoria] = self._palette[self._color_idx % len(
                self._palette)]
            self._color_idx += 1
    ...
    def _on_periodo_changed(self, idx: int):
        ...
        self.mes_ref.setVisible(
            self.combo_periodo.currentText().startswith("Mês"))
    ...
    def _atualizar_titulo_charts(self):
        if self._inicio and self._fim:
            if self._inicio.day == 1 and (
                    self._fim.month != self._inicio.month
                    or self._fim.day >= 28):
                # formato "Set/2025" quando for mês cheio
                texto = self._inicio.strftime("%b/%Y").capitalize()
            else:
                inicio_str = self._inicio.strftime('%d/%m/%Y')
                fim_str = self._fim.strftime('%d/%m/%Y')
                texto = f"{inicio_str} – {fim_str}"
        else:
